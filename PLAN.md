...
호가 이벤트를 블록 버퍼에 저장 → 워커가 심볼별 상태 객체 갱신 + 원본 이벤트 flush.
...
- 입력 버퍼: event_size/events_per_block을 버퍼 구조체에 직접 저장 (trcache 디커플링)
- 이벤트 버퍼 내부 단위: "chunk" 대신 "block" 용어 사용 (candle_chunk과 구분)
...
- book_state_ops: flush ops와 동일하게 void *ctx 패턴 사용
  - candle ops는 trcache 소유 구조체에 쓰므로 ctx 불필요
  - book_state_ops는 유저 소유 외부 상태에 접근하므로 ctx 필요
  - init/update/destroy 모두 ctx를 인자로 받음

---

## Phase 1: trade_data_buffer → event_data_buffer 리네이밍 + 디커플링 [완료]

**목적**: 버퍼를 trade 전용에서 범용으로 전환. `buf->trc->trade_data_size` 간접 참조를
`buf->event_size` 직접 참조로 변경.

### 1-1. 버퍼 구조체에 event_size, events_per_block, event_buf_size 필드 추가

**파일**: `src/include/pipeline/trade_data_buffer.h` → `event_data_buffer.h`로 리네이밍

- `struct trade_data_block` → `struct event_data_block`
...
- 새 필드 추가 (Group 2 read-only 영역):
  ...
  int    events_per_block;     // 블록당 이벤트 수
  size_t event_buf_size;       // 4KiB-aligned 데이터 버퍼 크기
  ```
- `enum trade_block_flush_state` → `enum event_block_flush_state`
- 매크로: `__get_trd_block_ptr` → `__get_evt_block_ptr`
...
- `calculate_data_buf_size()`: `tc->trades_per_block * tc->trade_data_size` → 인자로 받기
- `event_data_buffer_push()` (3곳):
  - L208: `buf->trc->trades_per_block` → `buf->events_per_block`
  - L257: `buf->trc->trade_data_buf_size` → `buf->event_buf_size`
  - L293-295: `buf->trc->trade_data_size` → `buf->event_size`
  - L309: `buf->trc->trades_per_block` → `buf->events_per_block`
- `event_data_buffer_peek()` (2곳):
  - L358: `buf->trc->trade_data_size` → `buf->event_size`
  - L364: `buf->trc->trades_per_block` → `buf->events_per_block`
...
```
init 함수 내부에서 `buf->event_size`, `buf->events_per_block`, `buf->event_buf_size` 설정.
...
| 파일 | 변경 내용 |
|------|-----------|
| `src/include/meta/trcache_internal.h` | `#include` 경로, `trade_data_size`/`trades_per_block`/`trade_data_buf_size` 필드는 유지 (init 시 계산용) |
| `src/meta/trcache_internal.c` | `trade_data_buffer_*` → `event_data_buffer_*` 함수 호출, `destroy_tls_data`의 block 타입 변경 |
...
`trade_data_size`, `trades_per_block`, `trade_data_buf_size`는 유지 (init 시 계산, TLS free list 해제 시 사용).
이 필드들이 버퍼에도 복사되지만, TLS destructor에서 free list의 block 크기를 알아야 하므로 trcache에도 남겨둠.
...
## Phase 1.5: GROUP_BATCH_FLUSH → GROUP_FLUSH 리네이밍 + flush 관련 변수명 정리 [완료]
...
## Phase 2: 캔들 콜백 시그니처에 book_state 추가 [완료]
...
```c
// 호가 상태 관리 콜백 (flush ops와 동일한 ctx 패턴)
...
**활성화 조건**: `book_state_ops.init != NULL`이면 book 파이프라인 활성화.
**참고**: `feed_block_size`를 trade/book 공용으로 사용. TLS free list도 공유.
...
- `book_enabled`이면:
  ...
  - `book_events_per_block = io_block_size / book_event_size` (trade와 동일 io_block_size 사용)
  - `book_event_buf_size = ALIGN_UP(book_events_per_block * book_event_size, 4096)`
...
`init_symbol_entry()`:
- `book_enabled`이면:
  - `event_data_buffer_init(tc, id, ...)` 호출
  - `entry->book_state = NULL` (lazy init: 첫 update 시 할당)

`symbol_table_destroy()`:
- `book_state != NULL`이면: `book_state_ops.destroy(entry->book_state, book_state_ops.ctx)`
    ...
    struct symbol_entry *entry)
{
    ...
    void *ctx = cache->book_state_ops.ctx;

    /* Lazy init: 첫 update 시 book_state 할당 */
    ...
    while (...peek(buf, cur, &array, &count)...) {
...
struct admin_state {
    ...
    uint64_t *book_block_fill_rates;       // [sym_idx]
...
## Phase 6: Makefile + 헤더 가드 정리 [완료 — Phase 1에서 처리됨]

## Phase 7: validator 수정 [완료 — Phase 2에서 처리됨]
...
```
Phase 1   (event_data_buffer 리네이밍)     [완료]
    ↓
Phase 1.5 (GROUP_FLUSH 리네이밍)           [완료]
    ↓
Phase 2   (콜백 시그니처)                  [완료]
    ↓
Phase 3   (book_state 인프라)              ← 다음
    ↓
Phase 4   (워커 태스크)
    ↓
Phase 5   (admin 모니터링)

Phase 6   (Makefile 정리)                  [완료]
Phase 7   (validator 수정)                 [완료]
```

## 위험 요소

1. ~~**Phase 1 리네이밍 범위**~~ — 해결됨.

2. ~~**콜백 시그니처 변경은 breaking change**~~ — 해결됨.

3. **event_data_buffer_flush_full_blocks의 flush ops 타입**: trade_flush_ops와
   book_event_flush_ops는 시그니처가 동일하지만 별도 타입. 내부적으로 flush 함수를
   호출할 때 void* 캐스팅이 필요하거나, 공통 flush ops 타입을 정의해야 함.

4. ~~**event_data_buffer의 cursor 수**~~ — 해결됨 (`num_cursor` init 파라미터).

5. **TLS free list 공유 전제**: trade와 book의 feed_block_size가 동일해야 함.
   trcache_init()에서 둘 다 같은 feed_block_size로 계산되도록 보장.
