...
typedef struct {
	 ...
	 * All timestamps are in nanoseconds (ns).
	 *
	 * exchange_ts: Timestamp generated by the exchange (T).
	 * feed_ts:     Timestamp just before engine injection.
	 * local_ts:    Timestamp when the candle was closed locally.
	 *
	 * Latency Breakdown:
	 * 1. Total Feed Latency = feed_ts - exchange_ts
	 * 2. Internal Latency   = local_ts - feed_ts
	 */
	uint64_t exchange_ts;
	uint64_t feed_ts;	/* [New] */
	uint64_t local_ts;
...
static const trcache_field_def val_candle_fields[] = {
	...
	{offsetof(val_candle, exchange_ts),  sizeof(uint64_t),
		FIELD_TYPE_UINT64},
	{offsetof(val_candle, feed_ts),      sizeof(uint64_t),
		FIELD_TYPE_UINT64},
